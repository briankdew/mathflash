...
function main() {
  ...
  for (const ef of eventFiles) {
    ...
    for (const [sidKey, events] of sessionGroups) {
      ...
      const accuracy = problems > 0 ? (100 * submitted) / problems : 0;
  ...
  for (const s of sessions) {
    ...
    console.log(
      ...
      `submission_eff=${toFixedOrNA(s.accuracy)}% acc_submitted=${toFixedOrNA(s.accuracySubmitted)}%`
    );
    console.log(
  ...
  for (const [mode, list] of byMode.entries()) {
    ...
    const acc = totals.problems > 0 ? (100 * totals.submitted) / totals.problems : 0;
    ...
    console.log(
      `  ${mode}: sessions=${totals.sessions} submission_eff=${toFixedOrNA(acc)}% acc_submitted=${toFixedOrNA(accSubmitted)}% ` +
  ...
  for (const [runId, list] of byRun.entries()) {
    ...
    const accShown = totals.problems > 0 ? (100 * totals.submitted) / totals.problems : 0;
    const accSubmitted = totals.submitted > 0 ? (100 * totals.correct) / totals.submitted : 0;
    const avg = (arr) => arr.length ? Math.round(arr.reduce((x, y) => x + y, 0) / arr.length) : null;
    const row = {
      run_id: runId,
    ...
    console.log(
      ...
      `submission_eff=${toFixedOrNA(accShown)}% acc_submitted=${toFixedOrNA(accSubmitted)}% ` +
