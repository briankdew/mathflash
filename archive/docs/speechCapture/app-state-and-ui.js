(() => {
  ...
  const VAA_VIEW_OPEN_HTML_ICON = 'sc-assets/icons/view-open-html.svg';
  ...
  function renderVaaTable() {
    ...
    vaaTableBody.innerHTML = vaaFiles.map((file, idx) => {
      ...
      const isHtmlFile = String(file.filename || '').toLowerCase().endsWith('.html');
      const viewIconSrc = isHtmlFile ? VAA_VIEW_OPEN_HTML_ICON : (s.view ? VAA_VIEW_OPEN_ICON : VAA_VIEW_CLOSED_ICON);
      const viewIconClass = isHtmlFile ? 'vaa-view-icon vaa-view-icon-html' : 'vaa-view-icon';
